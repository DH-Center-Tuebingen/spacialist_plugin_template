...
const availableOptions = {
    ...
    mainifest: {
        ...
        description: 'Prints the path to the `manifest.xml` file',
...
async function callManifest() {
    ...
    let succeeded = {};
    for (const file of ['manifest.xml', 'App/info.xml', 'lib/App/info.xml']) {
    ...
    if (!succeeded['App/info.xml'] && succeeded['lib/App/info.xml']) {
...
async function useManifest(path) {
    ...
    for (const path of ['manifest.xml', 'App/info.xml']) {
...
async function callLinkJs() {
    ...
    const srcPath = join(__dirname, 'dist', packageName + '.umd.js');
    try {
...
async function callVerifyVersion() {
    ...
    if (!manifestVersion) {
        error('The `manifest.xml` does not contain a version');
    }
    ...
    if (manifestVersion !== packageJson.version) {
        ...
        success('Version in `manifest.xml` matches version in package.json: v' + manifestVersion);
    }
}
